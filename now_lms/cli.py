...
def clear():
    ...
    with lms_app.app_context():
        ...
        if session_type != "sqlalchemy":
            click.echo(f"Session backend is '{session_type}', not 'sqlalchemy'. This command only works with SQLAlchemy session backend.")
            return
        ...
        try:
            ...
            result = db.session.execute(
                text(f"DELETE FROM {table_name} WHERE expiry <= :now"),
                {"now": datetime.utcnow()}
            )
...
def stats():
    ...
    with lms_app.app_context():
        ...
        try:
            ...
            result = db.session.execute(
                text(f"SELECT COUNT(*) FROM {table_name} WHERE expiry > :now"),
                {"now": datetime.utcnow()}
            )
