...
class SlideShowResource(database.Model, BaseTabla):
    ...
    slides = database.relationship("Slide", back_populates="slide_show", cascade=CASCADE_ALL_DELETE_ORPHAN, order_by="Slide.order")
...
class Question(database.Model, BaseTabla):
    ...
    evaluation_id = database.Column(database.String(26), database.ForeignKey(FOREIGN_KEY_EVALUATION_ID), nullable=False, index=True)
