...
def detail_public(slug):
    ...
    return render_template(
        "masterclass/detail_public.html",
...
def enroll(slug):
    ...
    return render_template(
        ...
        master_class=master_class,
        form=form,
        title=f"Inscribirse en {master_class.title}"
    )
...
def instructor_list():
    ...
    return render_template(
        "masterclass/instructor_list.html",
        master_classes=master_classes,
...
def instructor_create():
    ...
    return render_template(
        "masterclass/instructor_create.html",
        form=form,
        title="Crear Clase Magistral"
    )
...
def instructor_edit(id):
    ...
    if form.validate_on_submit():
        ...
        if form.title.data != master_class.title:
            ...
            counter = 1
            original_slug = slug
            existing = database.session.query(MasterClass).filter(
                and_(MasterClass.slug == slug, MasterClass.id != id)
            ).first()
            while existing:
                slug = f"{original_slug}-{counter}"
                counter += 1
    ...
    return render_template(
        "masterclass/instructor_edit.html",
        form=form,
        master_class=master_class,
...
def my_enrollments():
    ...
    return render_template(
        "masterclass/my_enrollments.html",
        enrollments=enrollments,
        title="Mis Clases Magistrales"
    )
...
def admin_list():
    ...
    return render_template(
        "masterclass/admin_list.html",
        master_classes=master_classes,
        title="Administrar Clases Magistrales"
    )