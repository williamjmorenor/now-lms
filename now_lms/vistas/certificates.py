...
from now_lms.db import MAXIMO_RESULTADOS_EN_CONSULTA_PAGINADA, Certificacion, Certificado, Curso, Usuario, MasterClass, MasterClassEnrollment, database
...
def certificacion_generar():
    """Generar un nuevo certificado."""
    from now_lms.db.tools import generate_cource_choices, generate_template_choices, generate_user_choices, generate_masterclass_choices
    ...
    if form.validate_on_submit() or request.method == "POST":
        ...
        if content_type == "course":
            ...
            enrollment = database.session.execute(
                ...
                .filter_by(master_class_id=form.master_class.data, user_id=form.usuario.data)
            ).first()
