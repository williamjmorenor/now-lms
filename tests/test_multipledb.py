...
pytestmark = pytest.mark.skipif(
    not should_run_multidb_tests(),
    reason="Tests de múltiples DB solo se ejecutan en CI con PostgreSQL o MySQL configurado"
)
...
class TestMultipleDatabaseSupport:
    ...
    def test_query_filtering(self, app, db_session):
        ...
        # Filtrar por tipo
        students = db_session.query(Usuario).filter_by(tipo="student").filter(
            Usuario.usuario.like("filtertest%")
        ).all()
        
        instructors = db_session.query(Usuario).filter_by(tipo="instructor").filter(
            Usuario.usuario.like("filtertest%")
        ).all()
