...
class TestCalendarUtilsComprehensive:
    ...
    def test_create_course_with_meets_via_api(self, full_db_setup, client):
        ...
        created_course = database.session.execute(
            select(Curso).filter_by(codigo='CAL001')
        ).scalar_one()
        ...
        meet_resources = database.session.execute(
            select(CursoRecurso).filter_by(curso='CAL001', tipo='meet')
        ).scalars().all()
    ...
    def test_create_course_with_evaluations_via_api(self, full_db_setup, client):
        ...
        evaluations = database.session.execute(
            select(Evaluation).join(CursoSeccion).filter(CursoSeccion.curso == 'CAL002')
        ).scalars().all()
    ...
    def test_user_enrollment_and_calendar_events(self, full_db_setup):
        ...
        section = CursoSeccion(
            curso='CAL003',
            nombre='Calendar Section',
            descripcion='Section for calendar testing',
            indice=1,
            estado=True
        )
        database.session.add(section)
        ...
        events_before = database.session.execute(
            select(UserEvent).filter_by(user_id=user.usuario)
        ).scalars().all()
        ...
        events_after = database.session.execute(
            select(UserEvent).filter_by(user_id=user.usuario, course_id='CAL003')
        ).scalars().all()
    ...
    def test_duplicate_event_prevention(self, full_db_setup):
        ...
        # Count events
        events_first = database.session.execute(
            select(UserEvent).filter_by(user_id=user.usuario, course_id='CAL004')
        ).scalars().all()
        ...
        events_second = database.session.execute(
            select(UserEvent).filter_by(user_id=user.usuario, course_id='CAL004')
        ).scalars().all()
    ...
    def test_update_meet_resource_events(self, full_db_setup):
        ...
        section = CursoSeccion(
            curso='CAL005',
            nombre='Update Section',
            descripcion='Update section',
            indice=1,
            estado=True
        )
        database.session.add(section)
        ...
        updated_event = database.session.execute(
        ...
        # Verify the update
        final_event = database.session.execute(
            select(UserEvent).filter_by(resource_id=meet_resource.id)
        ).scalar_one()
    ...
    def test_cleanup_events_for_course_unenrollment(self, full_db_setup):
        ...
        all_events = database.session.execute(
            select(UserEvent).filter_by(user_id=user.usuario)
        ).scalars().all()
        ...
        remaining_events = database.session.execute(
            select(UserEvent).filter_by(user_id=user.usuario)
        ).scalars().all()
    ...
    def test_error_handling(self, full_db_setup):
        ...
        events = database.session.execute(
            select(UserEvent).filter_by(user_id='nonexistent_user')
        ).scalars().all()
        assert len(events) == 0
    ...
    def test_edge_cases(self, full_db_setup):
        ...
        events = database.session.execute(
            select(UserEvent).filter_by(user_id=user.usuario, course_id='EDGE001')
        ).scalars().all()
    ...
    def test_update_evaluation_events_background(self, full_db_setup):
        ...
        section = CursoSeccion(
            ...
            descripcion='Eval update section',
            indice=1,
            estado=True
        )
        database.session.add(section)
        ...
        updated_event = database.session.execute(
            select(UserEvent).filter_by(evaluation_id=evaluation.id)
        ).scalar_one()
    ...
    def test_background_thread_functions(self, full_db_setup):
        ...
        section = CursoSeccion(
            curso='THREAD001',
            nombre='Thread Section',
            descripcion='Thread section',
            indice=1,
            estado=True
        )
        database.session.add(section)
    ...
    def test_comprehensive_integration(self, full_db_setup, client):
        ...
        section1 = CursoSeccion(
            curso='INTEG001',
            nombre='Week 1',
            descripcion='First week content',
            indice=1,
            estado=True
        )
        section2 = CursoSeccion(
            curso='INTEG001',
            nombre='Week 2',
            descripcion='Second week content',
            indice=2,
            estado=True
        )
        ...
        all_events = database.session.execute(
            select(UserEvent).filter_by(user_id=student.usuario, course_id='INTEG001')
