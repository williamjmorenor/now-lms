...
def test_user_registration_to_free_course_enroll(full_db_setup):
    ...
    with lms_application.app_context():
        ...
        # send_confirmation_email(user)  # Skip this call in test context
    ...
    # Resource must be marked as completed - check in separate app context
    with lms_application.app_context():
    ...
    # Final check on resource completion
    with lms_application.test_client() as client:
        # Login again
        client.post("/user/login", data={"usuario": "bmercado@nowlms.com", "acceso": "bmercado"})
    ...
    # User must be able to complete the course - check course progress
    with lms_application.app_context():
